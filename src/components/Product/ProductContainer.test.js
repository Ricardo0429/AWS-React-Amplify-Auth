...
describe( '(Container) Product', () => {
      ...
      it('Gets the product into the state if id found', () => {
            expect(props.dispatch.products.getOne.mock.calls.length).toEqual(1);
      ...
      it('Does not try to get the product if no id found', () => {
            ...
            expect(props2.dispatch.products.getOne.mock.calls.length).toEqual(0);
      });
      ...
      it('Passes a cb prop to update the product to Formik component', () => {
            ...
            wrapper.find(Formik).props().onSubmit('arg');
            ...
            expect(update.mock.calls[0][0].existingFile).toEqual('existing-file.png');
      });
      ...
      it('Passes filepath as prop to Product component', () => {
            ...
            const Form = wrapper.instance().renderForm({ setFieldValue: 'some method' });
      ...
      it('Passes a cb prop to silence alert to Product component', () => {
            const Form = wrapper.instance().renderForm({ setFieldValue: 'some method' });
            Form.props.onDrop();
            expect(props.dispatch.alert.silence.mock.calls.length).toEqual(1);
      });
      ...
      it('Passes editMode as prop to Product component', () => {
            let Form = wrapper.instance().renderForm({ setFieldValue: 'some method' });
            expect(Form.props.editMode).toEqual(true);
            wrapper.setProps({ match: { params: { id: null }}});
            Form = wrapper.instance().renderForm({ setFieldValue: 'some method' });
      ...
      it('Passes a cb prop to delete the product to Product component', () => {
            ...
            expect(props.dispatch.products.remove.mock.calls.length).toEqual(1);
            expect(props.dispatch.products.remove.mock.calls[0][0].id).toEqual(23);
            expect(props.dispatch.products.remove.mock.calls[0][0].filename).toEqual('some-file.png');
      });
      ...
      describe('Passes a cb prop to handle rejected files to Product component', () => {
            ...
            it('Displays an alert with a message indicating an unexpected error when cause not known', () => {
                  const Form = wrapper.instance().renderForm({ setFieldValue: 'some method' });
                  Form.props.onDropRejected([{ size: maxFileSize - 1, type: allowedFileTypes[0] }]);
                  expect(props.dispatch.alert.error.mock.calls.length).toEqual(1);
                  expect(props.dispatch.alert.error.mock.calls[0][0]).toEqual(unexpectedError);
            });
            ...
            it('Displays an alert with a message indicating the file size is too large', () => {
                  ...
                  Form.props.onDropRejected([{ size: maxFileSize + 1, type: allowedFileTypes[0] }]);
                  expect(props.dispatch.alert.error.mock.calls.length).toEqual(1);
                  expect(props.dispatch.alert.error.mock.calls[0][0]).toEqual(fileTooLarge(maxFileSize));
            });

            it('Displays an alert with a message indicating the file type is not allowed', () => {
                  const Form = wrapper.instance().renderForm({ setFieldValue: 'some method' });
                  Form.props.onDropRejected([{ size: maxFileSize - 1, type: 'image/notAllowed' }]);
                  expect(props.dispatch.alert.error.mock.calls.length).toEqual(1);
                  expect(props.dispatch.alert.error.mock.calls[0][0]).toEqual(fIleTypeError(allowedFileTypes.join(', ')));
            });
      });
      ...
      it('Passes a cb prop to handle accepted files to Product component', () => {
            ...
            Form.props.onDropAccepted([{ preview: 'file-preview', name: 'file-name' }]);
            ...
            expect(setFieldValue.mock.calls[0][1]).toEqual({ preview: 'file-preview', name: 'file-name' });
      });
});
